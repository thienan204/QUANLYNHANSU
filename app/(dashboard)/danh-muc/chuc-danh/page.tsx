...
export default function ChucDanhPage() {
    const { data: session } = useSession();
    const [items, setItems] = useState<ChucDanh[]>([]);
    const [isLoading, setIsLoading] = useState(true);
    const [isModalOpen, setIsModalOpen] = useState(false);
    const [editingItem, setEditingItem] = useState<ChucDanh | undefined>(undefined);
    ...
    const handleEdit = (item: ChucDanh) => {
        setEditingItem(item);
        setIsViewMode(false);
        form.setFieldsValue({ tenChucDanh: item.tenChucDanh });
        setIsModalOpen(true);
    };

    const handleView = (item: ChucDanh) => {
        setEditingItem(item);
        setIsViewMode(true);
        form.setFieldsValue({ tenChucDanh: item.tenChucDanh });
        setIsModalOpen(true);
    ...
    const handleDelete = async (id: number) => {
        try {
            const res = await fetch(`/api/chuc-danh/${id}`, { method: "DELETE" });
            if (res.ok) {
    ...
    const handleSave = async () => {
        try {
            ...
            const url = editingItem ? `/api/chuc-danh/${editingItem.id}` : "/api/chuc-danh";
    ...
    const columns: ColumnsType<ChucDanh> = [
        {
            title: 'ID',
            dataIndex: 'id',
            key: 'id',
        ...
        },
        {
            ...
            key: 'tenChucDanh',
            sorter: (a, b) => a.tenChucDanh.localeCompare(b.tenChucDanh),
        },
        {
            title: 'Hành động',
            key: 'action',
    ...
    return (
        <div className="container mx-auto p-4">
            ...
            <Modal
                title={isViewMode ? "Chi tiết Chức Danh" : (editingItem ? "Sửa Chức Danh" : "Thêm Chức Danh")}
                open={isModalOpen}
            ...
            >
                ...
                >
                    ...
                    >
                        {isViewMode ? <span className="font-semibold">{editingItem?.tenChucDanh}</span> : <Input placeholder="Nhập tên chức danh..." />}
                    </Form.Item>
                </Form>
            </Modal>
        </div>
